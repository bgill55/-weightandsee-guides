...
def generate_index_html():
    ...
    guides = []
    seen_titles = set()
    for item in sorted(os.listdir(GUIDES_DIR)):
        ...
        if os.path.isdir(item_path) and os.path.exists(os.path.join(item_path, "index.html")):
            ...
            if title not in seen_titles:
                ...
                guides.append({"id": item, "title": title})
            else:
                for i, g in enumerate(guides):
                    if g["title"] == title and len(item) > len(g["id"]):
                        guides[i] = {"id": item, "title": title}
                        break
                        
    # Sort them by date/time (newest first) using the get_guide_datetime helper
    guides.sort(key=lambda x: get_guide_datetime(x["id"]), reverse=True)
...
"""
    ...
    with open(index_path, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"guides/index.html generated with {len(guides)} guides.")
